...
export const checkIdValid = async (id: string): Promise<errResponse[]> => {
...
export const checkUpdateIdParam = async (
  ...
  if (!reqFile) {
    res.push({
      field: "id",
      message: "file doesn't belong to current user",
    });
    return res;
  }
  return res;
...
export const checkParentParam = async (
  newParent: string,
  ownerId: string,
  toMoveId: string
): Promise<errResponse[]> => {
...
export const checkFileNameParam = async (
  newFileName: string,
  ownerId: string,
  newParent: string
): Promise<errResponse[]> => {
